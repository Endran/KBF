...
public class AllWishListSummariesFragment extends BaseSummariesFragment {

	@Override
	protected void getSummaries(final ShowcaseLibrary showcaseLibrary, final SummariesFragmentView summariesFragmentView) {
		showcaseLibrary.getDataAccess().getAllWhisListSummariesAndCurrentUserRatings(new GetAllWhisListSummariesAndCurrentUserRatingsListener() {
			...
			public void onSummariesAndRatings(final List<Summary> summaries, final List<UserBeer> userBeers) {
				AllWishListSummariesFragment.this.summaries = summaries;
	...
	protected String getEmptyViewMessage(final Context context) {
		return context.getString(R.string.no_whishlist_yet);
	}
}
