...
public class GetSummariesByBeerrIdListQuery implements Query {

	private final List<Integer> beerIdList;

	public GetSummariesByBeerrIdListQuery(final List<Integer> beerIdList) {
	...
	public QueryContainer getQuery() {
		...
		if (beerIdList != null && beerIdList.size() > 0) {
			queryContainer.selection = TableHelper.TABLE_BEER.COLUMN_REMOTE_ID + "=" + beerIdList.get(0);
			for (int i = 1; i < beerIdList.size(); i++) {
				queryContainer.selection += " OR " + TableHelper.TABLE_BEER.COLUMN_REMOTE_ID + "=" + beerIdList.get(i);
			}
		}
